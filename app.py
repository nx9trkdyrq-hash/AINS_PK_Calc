...
from pk_models import DRUG_LIBRARY, build_regimen, validate_inputs, simulate_regimen, export_csv
...
with st.sidebar:
    ...
    height_cm = st.number_input("Größe (cm)", 100.0, 230.0, 175.0, 0.5)
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=result["time_min"], y=result["primary"]["cp"], mode="lines", name=f"{primary} Plasma"))
fig.add_trace(go.Scatter(x=result["time_min"], y=result["primary"]["ce"], mode="lines", name=f"{primary} Effektort"))
...
fig.update_layout(template="plotly_white", height=620, xaxis_title="Zeit (min)", yaxis_title="Konzentration")
...
st.download_button(label="CSV exportieren", data=export_csv(result), file_name="anesthesia_pk_export.csv", mime="text/csv")
